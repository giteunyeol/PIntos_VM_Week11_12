...
/* 전역 디스크립터 테이블(GDT).
 *
 * GDT는 x86-64 전용 구조체로, 권한에 따라 시스템의 모든 프로세스가 사용할
 * 수 있는 세그먼트를 정의한다. 프로세스별 로컬 디스크립터 테이블(LDT)도
 * 존재하지만, 현대 운영체제에서는 사용하지 않는다.
 *
 * 테이블 안에서 바이트 오프셋으로 식별되는 GDT의 각 엔트리는 하나의
 * 세그먼트를 뜻한다. 여기서 중요한 세그먼트는 세 가지뿐인데, 코드,
 * 데이터, 그리고 TSS(Task-State Segment) 디스크립터다. 앞의 두 개는
 * 이름 그대로이고, TSS는 주로 인터럽트 처리 시 스택 전환에 사용된다. */
...
/* 올바른 GDT를 구성한다. 부트스트랩 로더의 GDT에는 사용자 모드 셀렉터와
